...
export default class Manutencao extends Component {
    ...
    componentDidMount() {  
        return fetch(Url + "manutencao.php")
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    <SafeAreaView>
                        ...
                        <Picker
                              ...
                              onValueChange={(dadosValue, itemIndex) =>alert('oi')}>
                                <Picker.Item label="Escolha um país"/>
                              {this.state.dados.map((item, key) => (
                                <Picker.Item label={item.empresa} value={item.empresa} key={key} />)
                              )}
                            </Picker> 
                        <View style={{ marginLeft: wp('0.5%'), width:wp('99%'), flexDirection:'row', flex:1, justifyContent:"space-between"}}> 
                            ...
                            <Table style={{width: wp('43%')}} borderStyle={{ borderWidth: 2, borderColor: '#C1C0B9' }}> 
                                <Rows widthArr={this.state.widthTab2} data={this.state.dados.empresa} textStyle={styles.text}/>
                            </Table>
                            </View>
